 ...
 * ID           - the id of the layer itself. If you want to change or remove
 * GRAPHIC      - The filename of the image found in /img/layers/
 * X            - Where the left of the layer is positioned on the map.
 *              - 0 is the far left of the map
 * Y            - Where the top of the layer is positioned on the map.
 *              - 0 is the very top of the map
 * OPACITY      - Transparency of the image (0 - 255)
 * Z            - What priority the image has. 0 = ground, 5 = above all chars
 * BLEND        - Blend mode (0 = normal, 1 = add, 2 = multiply, 3 = screen)
 *              - *NOTE RPG Maker MV version 1.1 mutliply isn't supported
 ...
 *
 *   LAYER_S ID GRAPHIC X Y OPACITY Z BLEND
 *
...
Sprite_LayerGraphicS.prototype.createBitmap = function() {
...
// Update Position
Sprite_LayerGraphicS.prototype.updatePosition = function() {
	this.z = this.lValue().z || 0;
	this.opacity = this.lValue().opacity || 0;
	this.blendMode = this.lValue().blend || 0;
	
	this.x = this.lValue().x - $gameMap.displayX() * Galv.LG.tileSize;
	this.y = this.lValue().y - $gameMap.displayY() * Galv.LG.tileSize;
};
